...
(/api/tarjas/detalle/download-pdf) must show the same Resumen table and
pie chart as the on-screen "Detalle de la semana" report, with the
percentage visible directly on each chart slice (not only in a tooltip,
which a static PDF has no way to reproduce).
"""
...
class TestTipoPagoHelpers:
    ...
    def test_96_color_trato_blue_rest_orange(self):
        assert tc._tipo_pago_color("trato") == "#3b82f6"
        assert tc._tipo_pago_color("Al dia") == "#f97316"
        assert tc._tipo_pago_color("Tractorista") == "#f97316"
        assert tc._tipo_pago_color("Bono") == "#f97316"


class TestAsciiFold:
    def test_96_strips_spanish_accents(self):
        assert tc._ascii_fold("Día") == "Dia"
        assert tc._ascii_fold("Al Día") == "Al Dia"

    def test_96_leaves_plain_ascii_unchanged(self):
        assert tc._ascii_fold("Tractorista") == "Tractorista"
...
class TestPieChartB64:
    def test_96_returns_png_for_nonempty_resumen(self):
        resumen = [
            {"tipo_pago": "trato", "total_pagar": 1000000, "jornadas": 10},
            {"tipo_pago": "Al dia", "total_pagar": 500000, "jornadas": 5},
        ]
        b64 = tc._pie_chart_b64(resumen)
        assert b64
        import base64
    ...
    def test_96_empty_resumen_returns_empty_string(self):
        assert tc._pie_chart_b64([]) == ""

    def test_96_zero_total_returns_empty_string(self):
        resumen = [{"tipo_pago": "trato", "total_pagar": 0, "jornadas": 0}]
        assert tc._pie_chart_b64(resumen) == ""
...
class TestDownloadPdfIntegration:
    ...
    def test_96_detalle_pdf_includes_resumen_and_chart_regression(self):
        """The original bug: the Detalle Operacional PDF only rendered the
        Detalle table, missing the Resumen table and pie chart shown on
        screen. This must no longer be the case, and the PDF must not raise."""
        resp = self._run(
        ...
        # The pie chart must be embedded as an image (xhtml2pdf can't
        # execute <canvas>, so it has to be a flat PNG — see _pie_chart_b64).
